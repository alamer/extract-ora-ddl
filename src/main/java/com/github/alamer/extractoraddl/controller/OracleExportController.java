...
public class OracleExportController {
    ...
    public String exportDdlByViewName(String viewName) throws ClassNotFoundException, SQLException, IOException {
        Class.forName("oracle.jdbc.driver.OracleDriver");
        StringBuilder b = new StringBuilder();
        try (Connection conn = getConnection()) {
            b.append(getView(conn, viewName));
            return b.toString();
        }
    }
    ...
    private String getView(Connection conn, String tableName) throws IOException, SQLException {
        StringBuilder builder = new StringBuilder();
        String sql = new String(Files.readAllBytes(Paths.get("sql/exportView.sql")));
        return executeExport(conn, tableName, sql);
    }
